...
def validate_duplicate_detection(history_data: List[Dict]) -> Dict:
    """Valida l'accuratezza del rilevamento duplicati"""
    validation = {
        'total_tested': 0,
        'true_positives': 0,
        'false_positives': 0,
    ...
    # Raggruppa per vehicle_id
    for listing_id in df['listing_id'].unique():
        listing_data = df[df['listing_id'] == listing_id]
        
        # Verifica duplicati identificati
        if 'duplicate_of' in listing_data.columns:
            duplicates = listing_data[listing_data['duplicate_of'].notna()]
            
            for _, dup in duplicates.iterrows():
                validation['total_tested'] += 1
                
                # Verifica matching
                match_score = verify_duplicate_match(
                    dup,
                    df[df['listing_id'] == dup['duplicate_of']].iloc[0]
                )
                ...
                if match_score >= 0.8:
                    ...
                    validation['problem_cases'].append({
                        'listing_id': dup['listing_id'],
                        'duplicate_of': dup['duplicate_of'],
                        'match_score': match_score,
                        'reason': get_mismatch_reason(dup, df[df['listing_id'] == dup['duplicate_of']].iloc[0])
                    })
    
    return validation

def verify_duplicate_match(listing1: pd.Series, listing2: pd.Series) -> float:
    """Verifica accuratezza del match tra duplicati"""
    score = 0.0
    checks = 0
    
    # Check targa
    if listing1.get('plate') and listing2.get('plate'):
        if listing1['plate'] == listing2['plate']:
            score += 1.0
        checks += 1
    ...
    if listing1.get('original_price') and listing2.get('original_price'):
        ...
        if price_diff / max(listing1['original_price'], listing2['original_price']) < 0.1:
            score += 1.0
        checks += 1
        
    # Check immagini
    if ('image_urls' in listing1 and 'image_urls' in listing2 and 
        listing1['image_urls'] and listing2['image_urls']):
        img_similarity = compare_images(
            listing1['image_urls'][0], 
            listing2['image_urls'][0]
        )
        if img_similarity > 0.8:
            score += 1.0
        checks += 1
        
    return score / checks if checks > 0 else 0.0

def get_mismatch_reason(listing1: pd.Series, listing2: pd.Series) -> str:
    """Identifica il motivo del mismatch tra duplicati"""
    reasons = []
    
    if listing1.get('plate') != listing2.get('plate'):
        reasons.append("Targhe diverse")
        
    if listing1.get('original_price') and listing2.get('original_price'):
        price_diff = abs(listing1['original_price'] - listing2['original_price'])
        if price_diff / max(listing1['original_price'], listing2['original_price']) >= 0.1:
            reasons.append("Differenza prezzo significativa")
    ...
    if ('image_urls' in listing1 and 'image_urls' in listing2 and 
        ...
        if img_similarity <= 0.8:
            reasons.append("Immagini diverse")
            
    return ", ".join(reasons) if reasons else "Motivo non identificato"